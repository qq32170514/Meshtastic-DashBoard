...
export default function CwaNodeMap({ nodes, regionSummary = [], darkMode }: CwaNodeMapProps) {
  ...
  return (
    <div className="space-y-4">
      {/* 🏙️ 各縣市氣象對比圖卡專區 (One Weather Card per County) */}
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-300">
            <span className="text-sm">⛅</span> 各縣市氣象戰情圖卡
              ...
              >
                ...
                <div className="space-y-1.5 text-xs font-mono">
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400 text-[11px]">Mesh 節點實測均溫</span>
                    <span className="font-bold text-amber-400">{fmt(card.avgMeshTemp)} °C</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400 text-[11px]">CWA 官方氣象站</span>
                    <span className="font-bold text-blue-400">{fmt(card.avgCwaTemp)} °C</span>
                  </div>

                  {/* 區域 ΔT 標籤 */}
                  <div className="flex justify-between items-center pt-1 border-t border-slate-700/40">
                    <span className="text-slate-400 text-[11px]">區域平均 ΔT</span>
                    <span
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-[10px] font-mono">
          <div className="flex items-center gap-3">
            <span className={darkMode ? 'text-slate-400' : 'text-slate-500'}>地圖圖例：</span>
            <span className="flex items-center gap-1.5"><span className="inline-block w-2.5 h-2.5 rounded-full bg-green-500"></span>ΔT &lt; 1.5°C 正常</span>
            <span className="flex items-center gap-1.5"><span className="inline-block w-2.5 h-2.5 rounded-full bg-yellow-400"></span>1.5–3°C 偏高</span>
            <span className="flex items-center gap-1.5"><span className="inline-block w-2.5 h-2.5 rounded-full bg-orange-500"></span>3–5°C 異常偏高</span>
            <span className="flex items-center gap-1.5"><span className="inline-block w-2.5 h-2.5 rounded-full bg-red-500"></span>≥ 5°C 嚴重過熱</span>
            <span className="flex items-center gap-1.5"><span className="inline-block w-2.5 h-2.5 rounded-full border-2 border-blue-400" style={{ background: 'rgba(59,130,246,0.3)' }}></span>CWA 氣象站</span>
          </div>
          {selectedCounty && (
            <span className="text-cyan-400 font-bold">
              目前篩選：{selectedCounty} ({filteredNodes.length} 個節點)
            </span>
          )}
        </div>

        <div className="rounded-2xl overflow-hidden border border-slate-800 shadow-xl" style={{ height: '500px' }}>
          <MapContainer
              ...
              >
                <LeafletTooltip direction="top" offset={[0, -8]}>
                  <div style={{ fontSize: '11px', lineHeight: '1.7' }}>
                    ...
                    官方氣溫：<strong>{fmt(st.temp)} °C</strong><br />
              ...
              >
                <LeafletTooltip direction="top" sticky offset={[0, -6]}>
                  <div style={{ fontSize: '11px', lineHeight: '1.8', minWidth: '190px' }}>
                    ...
                    {n.nodeHumidity != null ? <span style={{ color: '#94a3b8' }}> / {fmt(n.nodeHumidity)}%</span> : null}<br />
                    比對站：{n.cwaStationName}<br />
                    <span style={{ color: '#94a3b8', fontSize: '10px' }}>{n.cwaCounty} {n.cwaTown} · {fmt(n.distanceKm)} km</span><br />
                    官方氣溫：<strong style={{ color: '#60a5fa' }}>{fmt(n.cwaTemp)} °C</strong><br />
