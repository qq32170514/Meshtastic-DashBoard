      ...
      {(cwaNodeComparison || cwaComparison) && (
        <div className={`p-5 rounded-2xl border shadow-sm space-y-4 ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          ...
          {cwaNodeComparison && cwaViewMode === 'map' && (() => {
            const nodes = cwaNodeComparison.nodeComparisons;
            if (nodes.length === 0) return <div className="text-center text-slate-500 py-10">沒有節點數據可顯示</div>;
            ...
            // ΔT → 顏色
            const getDeltaColor = (dt: number | null): string => {
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-4 text-[10px] font-mono">
                  ...
                  <span className="text-red-400">
                    ⚠️ 異常節點 {nodes.filter(n => n.anomaly).length} 個
                  </span>
                  <span className="text-green-400">
                    ✓ 正常節點 {nodes.filter(n => !n.anomaly && n.deltaTemp !== null).length} 個
                  </span>
                </div>
              </div>
            );
