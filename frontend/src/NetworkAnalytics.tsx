...
// ==========================================
// 🗺️ CWA 節點配對地圖 Component
// 獨立 Component 避免 Leaflet L.Map 構造函數衝突 (Hd is not a constructor)
// ==========================================
...
function CwaNodeMap({ nodes, darkMode }: { nodes: NodeComparison[]; darkMode: boolean }) {
  if (nodes.length === 0) {
    return <div className="text-center text-slate-500 py-16">沒有節點數據可顯示</div>;
  }
  ...
  nodes.forEach(n => {
    if (!cwaStationMap.has(n.cwaStationId)) {
      cwaStationMap.set(n.cwaStationId, {
        id: n.cwaStationId, name: n.cwaStationName,
        county: n.cwaCounty, town: n.cwaTown,
        lat: n.cwaLat, lng: n.cwaLng,
        temp: n.cwaTemp, weather: n.cwaWeather,
      });
    }
  });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] font-mono">
        ...
        <span className="flex items-center gap-1.5"><span className="inline-block w-5" style={{ borderTop: '2px dashed #64748b' }}></span>配對連線</span>
      </div>
        ...
        >
          ...
          {nodes.map(n => (
            <Polyline
              ...
              pathOptions={{ color: getDeltaColor(n.deltaTemp), weight: 1.5, opacity: 0.45, dashArray: '5,7' }}
            />
          ))}
          ...
          {uniqueCwaStations.map(st => (
            <CircleMarker
              ...
              radius={7}
              pathOptions={{ color: '#93c5fd', fillColor: '#3b82f6', fillOpacity: 0.45, weight: 2 }}
            >
              <LeafletTooltip direction="top" offset={[0, -8]}>
                <div style={{ fontSize: '11px', lineHeight: '1.7' }}>
                  <strong>🌡️ {st.name}</strong><br />
                  {st.county} {st.town}<br />
                  官方氣溫：<strong>{st.temp} °C</strong><br />
            ...
            >
              <LeafletTooltip direction="top" sticky offset={[0, -6]}>
                <div style={{ fontSize: '11px', lineHeight: '1.8', minWidth: '190px' }}>
                  ...
                  節點實測：<strong style={{ color: '#fbbf24' }}>{n.nodeTemp ?? '--'} °C</strong>
                  {n.nodeHumidity != null ? <span style={{ color: '#94a3b8' }}> / {n.nodeHumidity}%</span> : null}<br />
                  ...
                  <strong style={{ color: n.anomaly ? '#f87171' : n.deltaTemp != null && n.deltaTemp > 0 ? '#fb923c' : '#4ade80', fontSize: '12px' }}>
                    ΔT = {n.deltaTemp != null ? (n.deltaTemp > 0 ? '+' : '') + n.deltaTemp + ' °C' : '--'}
                    {n.anomaly ? '  ⚠️ 異常過熱' : ''}
                  </strong>
                </div>
              </LeafletTooltip>
      ...
      <div className="flex flex-wrap gap-4 text-[10px] font-mono">
        <span className={darkMode ? 'text-slate-400' : 'text-slate-500'}>
          共 {nodes.length} 個 Mesh 節點 · 配對 {uniqueCwaStations.length} 個 CWA 氣象站
        </span>
